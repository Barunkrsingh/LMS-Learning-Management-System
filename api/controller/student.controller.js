...
module.exports = {
    ...
    getStudentWithQuery: async(req, res)=>{
        ...
        try {
            ...
            if(req.query.hasOwnProperty('student_semester')){
                filterQuery['student_semester'] = req.query.student_semester
            }
    
            const filteredStudents = await Student.find(filterQuery).populate("student_semester");
                ...
                if (resp.length > 0) {
                    ...
                    fs.writeFile(newPath, photoData, function (err) {
                        ...
                        const newStudent = new Student({
                            ...
                            name: fields.name[0],
                            student_semester:fields.student_semester[0],
            ...
            if (resp.length > 0) {
                ...
                if (isAuth) {   
                    const token = jwt.sign(
                        {
                            id: resp[0]._id,
                            departmentId: resp[0].department,
                            email: resp[0].email,
    ...
    getStudentWithId: async(req, res)=>{
        ...
        const departmentId =  req.user.departmentId;
        Student.findOne({_id:id, department:departmentId}).populate("student_semester").then(resp=>{
            if(resp){
    ...
    //         await Student.findOneAndUpdate({_id:id,department:departmentId},{$set:{...req.body}});
...
updateStudentWithId : async (req, res) => {
    ...
    deleteStudentWithId: async(req, res)=>{
        try {
            let id = req.params.id;
            const departmentId =  req.user.departmentId;
            await Attendance.deleteMany({department:departmentId,student:id})
            await Student.findOneAndDelete({_id:id, department:departmentId,});
