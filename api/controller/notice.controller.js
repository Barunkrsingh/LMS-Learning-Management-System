...
exports.newNotice = async (req, res) => {
  ...
  const semesterId = req.user.semesterId;
  try {
    const newNotice = new Notice({ title, message, audience, semester:semesterId });
...
exports.fetchAudiance = async (req, res) => {
  ...
  const semesterId = req.user.semesterId;
  try {
    const notices = await Notice.find({ audience,semester:semesterId });
    res.json(notices);
  } catch (error) {
...
exports.fetchAllAudiance = async (req, res) => {
  const semesterId = req.user.semesterId;
    try {
      const notices = await Notice.find({semester:semesterId});
      res.json(notices);
    } catch (error) {
      console.error("Error fetching notices:", error);
      res.status(500).json({ message: "Error fetching notices." });
