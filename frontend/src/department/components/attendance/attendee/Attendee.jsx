...
export default function Attendee({ semesterId, handleMessage,params}){
    ...
    const initialValues = {
      ...
      const Formik = useFormik({
        ...
        onSubmit: (values) => {
          axios
            .patch(`${baseUrl}/semester/update/${semesterId}`, { ...values })
            .then((resp) => {
      ...
      const fetchSemesterWithId = () => {
        ...
        if (semesterId) {
          ...
          axios
            .get(`${baseUrl}/semester/fetch-single/${semesterId}`)
            .then((resp) => {
              console.log("Single semester", resp.data.data);
      ...
      useEffect(()=>{
        fetchAllTeachers();
       fetchSemesterWithId();
      },[iseditAttendee,semesterId,params])
    return (
        <>
